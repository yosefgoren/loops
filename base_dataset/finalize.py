...
SYSTEM_PROMPT = "You are a specialized chatbot trained to estimate the parallel execution efficiency of given OpenMP parallelized code snippets. Your task is to classify how well the code scales as one of five categories:\n1 - Negative: Parallelization in very inefficient and is slower than serial version.\n2 - Nutral: Parallelization yields similar runtime performance.\n3 - Low: Parallilization yields a slight runtime improvement.\n4 - Medium: Parallelization yields a moderate runtime improvement.\n5 - High: Parallelization yields a significant speedup.\n"
NUMBER_OF_CLASSES = 5
TRAIN_VALID_RATIO = 0.8

def classify_results_uniform(coefs: list[float], nb_classes: int) -> list[int]:
    """
    This function decides how different performance coefs should be classified to different performance categories.
    ...
    The categorization is relative to the list of inputs itself, and is defind by percentiles
        where the classes of percenties are all of uniform proportion to the entier dataset (list of coefs).
...
def finalize(output_prefix: str, input_files: list[str]) -> None:
    ...
    raw_samples: list[LoopCoefficent] = []
    for fname in input_files:
        raw_samples += load_coefficients_file(fname)

    # Filter the samples for ones with 'None' duration:
    filtered_samples = [sample for sample in raw_samples if sample.coeffient is not None]
    durations: list[float] = [sample.coeffient for sample in filtered_samples] # type: ignore
